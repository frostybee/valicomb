...
The `regex` rule includes automatic protection against dangerous patterns:
...
// This throws RuntimeException - dangerous pattern detected
...
Language file loading validates against directory traversal attacks:

```php
// This throws InvalidArgumentException - path traversal blocked
new Validator([], [], '../../etc/passwd');

// This throws InvalidArgumentException - invalid language code
new Validator([], [], '../../../evil');
```

### Language Code Validation

Only alphanumeric language codes with optional hyphens are accepted:
- ✅ `en`, `en-US`, `zh-Hans`
- ❌ `../etc`, `../../`, `.`
...
$v->rule('email', 'email');        // ✅ Validates email format
$v->rule('lengthMax', 'email', 5); // ❌ Just checks length
```
