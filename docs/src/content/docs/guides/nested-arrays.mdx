...
Errors are keyed by the full field path:
...
$errors = $v->errors();
// [
//     'users.1.email' => ['users.1.email is not a valid email address']
// ]
```
...
// Or more specific
$v->labels([
    'users.0.email' => 'Primary Contact Email',
    'users.1.email' => 'Secondary Contact Email'
]);
```
