...
The core workflow is simple: create a Validator with your data, define rules for each field, and call `validate()`. If validation fails, you can retrieve the error messages.
...
Valicomb also supports fluent chaining (`$v->rule('email', 'email')`) and rule-based arrays. See [Defining Rules](/valicomb/getting-started/defining-rules/) for all options.
:::
